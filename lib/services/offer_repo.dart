...
class OfferRepository{
  ...
  Future<List<OfferModel>> getAllOffers() async {
    try {
      final response = await _dio.get('/OffersConrtoller/GetAllOffers');

      if (response.statusCode == 200) {
        final List<dynamic> jsonList = response.data;
        print('object5');
        return jsonList.map((json) => OfferModel.fromJson(json)).toList();
      } else {
      ...
      throw Exception('Failed to load scholarss: $e');
    }
  }

  // Get single scholar by ID
  Future<OfferModel> getOffersById(int id) async {
    try {
      final response = await _dio.get('/OffersConrtoller/FindOfferBy$id');
      ...
      if (response.statusCode == 200) {
        ...
        if (response.data is List) {
          ...
          return OfferModel.fromJson(tripData);
